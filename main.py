...
class DistanceField(object):
    ...
    def get_block(self, x, z, y):
        ...
        fx = math.fmod(x, step) - sx * step / 2
        fz = math.fmod(z, step) - sz * step / 2
        nx = fx + 2 * math.sin(z / 4.7 + math.cos(fx))
        nz = fz + 2 * math.cos(x / 5.9 + math.sin(fz))
        if z != 0:
            nx += math.cos(math.fmod(x * x, z))
        if x != 0:
            nz += math.sin(math.fmod(z * z, x))
        ny = y + math.sin(0.75 + math.cos(x / 7.8) + math.sin(z / 9.1))

        if math.sqrt(nx * nx + nz * nz + ny * ny * 64) < 32:
            return c.MAT_DIRT
